...
class S3Manager:
    ...
    def download_available_predictions(self, coin, model_name):
        ...
        create_dir(f"/opt/airflow/custom_persistent_shared/data/predictions/{coin}")
        create_dir(f"/opt/airflow/custom_persistent_shared/data/predictions/{coin}/{model_name}")
        
        print(f"Existing versions for {coin}: {existing_versions}")
        for v in existing_versions:
            local_file = f"/opt/airflow/custom_persistent_shared/data/predictions/{coin}/{model_name}/v{v.split('/')[-1].split('.')[0][1:]}.csv"
    ...
    def update_local_pred(self, coin, model_name, upload_s3=True):
        ...
        if chosen_version is not None:
            local_file = f"/opt/airflow/custom_persistent_shared/data/predictions/{coin}/{model_name}/v{chosen_version.split('/')[-1].split('.')[0][1:]}.csv"
