...
for coin in cryptos:
    ...
    train_df["open_time"] = pd.to_datetime(train_df["open_time"], utc=True, format='mixed')
    test_df["open_time"] = pd.to_datetime(test_df["open_time"], utc=True, format='mixed')
    ...
    print(f"Full start date: {full_df.iloc[0]['open_time']}, Full end date: {full_df.iloc[-1]['open_time']}")
    print(f"Test size: {len(test_df)}")
    
    train_df, test_df = full_df[len(test_df):], full_df[:len(test_df)]
    # if len(train_df) > RETENTION_MIN:
    #     train_df = train_df[-RETENTION_MIN:]
